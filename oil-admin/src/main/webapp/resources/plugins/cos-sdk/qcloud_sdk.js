...
CosCloud.prototype.getAppSign = function(success, error, bucketName){
	...
	var url = this.sign_url + "/cos/sign/appSign";
	$.ajax({
		url : url,
		type : "POST",
		data : {
...
CosCloud.prototype.getAppSignOnce = function(success, error, path, bucketName){
	var url = this.sign_url + "appSignOnce";	
	$.ajax({
		url : url,
		type : "POST",
		data : {
...
CosCloud.prototype.updateFile = function(success, error, bucketName, remotePath, bizAttribute, authority, customHeaders){
	...
	this.getAppSignOnce(function(json){		
		var jsonResult = $.parseJSON(json);alert(jsonResult);
...
CosCloud.prototype.uploadFile = function(divIndex, success, error, bucketName, remotePath, file, insertOnly){
	var that = this;
	this.getAppSign(function(json){
		var sign = json.sign;
		var appid = json.appID;
		var url = that.cosapi_cgi_url + appid + "/" + bucketName + encodeURI(remotePath) + "?sign=" + encodeURIComponent(sign);
		var formData = new FormData();
		...
		var uploadSuccess = function(result){
			...
			if(jsonResult.data.offset != undefined){
				...
				document.getElementById("uploadFileNameDiv"+divIndex).innerHTML = document.getElementById("uploadFileNameDiv"+divIndex).getAttribute("attrname")+'&nbsp;'+(offset/file.size*100).toFixed(2)+'%';
				sliceUpload();
			}
			else{
				document.getElementById("uploadFileNameDiv"+divIndex).innerHTML = document.getElementById("uploadFileNameDiv"+divIndex).getAttribute("attrname")+'&nbsp;上传完成!';
				//success(result);
				return;
			}
		};
...
CosCloud.prototype.sliceUploadFile = function(divIndex,success, error, bucketName, remotePath, file, insertOnly, optSliceSize){
	var that = this;
	...
	reader.onload = function(e){
        if(e.target.result != null) {
            ...
            if (e.target.result.length != 0) {
            	if(!Qh){
            		Qh = swfobject.getObjectById("qcloudservice"+divIndex);
            	}
	...
	var Qh = swfobject.getObjectById("qcloudservice"+divIndex);
