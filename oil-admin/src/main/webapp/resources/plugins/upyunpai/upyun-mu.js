...
'use strict';
    ...
    function upload(value, fileId,fileSelector) {
        var self = this;
...
function PreviewImg(obj,id) {  
    ...
    var fileId = obj.id;              
    var labelId = $('#'+fileId).attr("field");
    $("#"+id).attr("src",imgPath).after( "<a style='margin-left:30px;' class='btn bg-olive' onclick='upImage(\""+id+"\",\""+fileId+"\",\""+labelId+"\")'> <i class='fa fa-plus'></i>上传</a>" );
}
//上传图片
function upImage(imgId,fileId,label){
	$("#"+imgId).next().remove();
	$("#"+imgId).after("<a style='margin-left:30px;' class='btn bg-olive'> <i class='fa fa-plus'></i>上传成功</a>");
	var config = {
		bucket: 'aikxian',
		expiration: parseInt((new Date().getTime() + 3600000) / 1000),
		// 尽量不要使用直接传表单 API 的方式，以防泄露造成安全隐患
		form_api_secret: 'pLvYKo1tWP3A3SxYt2Do3uSMyx8='
	};
	var instance = new Sand(config);
	var options = {
		'notify_url': 'http://upyun.com'
	};
	var ext = '.' + document.getElementById(''+fileId).files[0].name.split('.').pop();
	var picURL='/images/liveRoom/'+parseInt((new Date().getTime() + 3600000) / 1000) + ext;
	instance.setOptions(options);
	instance.upload(picURL,fileId);
	$("input[name='"+label+"']").val("http://aikxian.b0.upaiyun.com"+picURL);
